...
```
Сначала устраняем предупреждение, вызванное perl-5.22 и более поздними версиями:
```
sed -i 's:\\\${:\\\$\\{:' intltool-update.in
```
Приведенное выше регулярное выражение выглядит необычно из-за обилия обратных косых черт. Оно добавляет обратную косую черту перед закрывающей фигурной скобкой в ​​последовательности `\${`, в результате чего получается `\$\{`.  
  
Конфигурируем Intltool для компиляции
```
./configure --prefix=/usr
```
Компилируем пакет
