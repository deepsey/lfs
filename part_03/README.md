...
0
```
Переменная DESTDIR используется почти всеми пакетами для определения места, куда пакеты будут установлены. Если она не определена, то ее значение равно / (root директория).  
